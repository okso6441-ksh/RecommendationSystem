...
* Bayesian Personalized Ranking(BPR) framework 훈련 절차 개발  
...
* 선호도 예측기: MF 기반(Matrix Factorization)  
  * ![(1)](./image/(1).PNG)  
    * α: 전역 오프셋  
    * $ \beta_u, \beta_i $:사용자, 항목 편향  
    * $ \gamma_u, \gamma_i $: 사용자, 항목 잠재 인자 설명 K 차원 벡터  
    * $ \gamma_u^T\gamma_i $: 내적; 사용자 u - 항목 i 사이 '호환성(compatibility)'; 사용자 잠재  '선호'가 제품 '속성' 일치하는 정도를 인코딩   
...
* 확장된 예측자 형식  
  * ![(2)](./image/(2).PNG)  
    * $ \theta_u, \theta_i $: new D차원 시각적 factors  
...
* DS: training set; 트리플(u, i, j): 사용자 u - 긍정 피드백 표현 item i & 관찰되지 않는 item j   
  * ![(5)](./image/(5).PNG)  
    * Θ: 매개 변수 벡터  
    * $ \hat x_{uij}(\Theta) $: 트리플(u, i, j)의 구성 요소 간 관계 매개 변수화 Θ의 임의 함수  
* 최적화 기준: 개인화 ranking(BPR-OPT)  
  * ![(6)](./image/(6).PNG)  
    * σ: 로지스틱(시그모이드) 함수  
    * $ \lambda_\Theta $: 모델 별 regularization hyperparameter  
...
* 제안된 모델의 BPR-MF 대비 업데이트 매개변수 세트(2):    
  ...
  * (b) 새로 도입 된 시각적 매개변수  
    * ![1](./image/1.PNG)  
      * $ \lambda_E $: hyperparameter; 임베딩 행렬 E regularize  
